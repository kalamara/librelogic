...
int main() {
    ...
    if (NULL == suite_lib || NULL == suite_io || NULL == suite_il || NULL == suite_ld || NULL == suite_tree || NULL == suite_codegen || NULL == suite_init) {
        CU_cleanup_registry();
        return CU_get_error();
    }
    ...
    // start_thread();
//plclib
    if (ADD_TEST(suite_lib, ut_codec) || ADD_TEST(suite_lib, ut_stack)
...
//IL lexer-codegen
    if (ADD_TEST(suite_il, ut_number) || ADD_TEST(suite_il, ut_char)
...
//syntax tree
    if (ADD_TEST(suite_tree, ut_mk_identifier) || ADD_TEST(suite_tree, ut_mk_expression) || ADD_TEST(suite_tree, ut_mk_assignment)) {
        CU_cleanup_registry();
        return CU_get_error();
    }

//code generator
    if (ADD_TEST(suite_codegen, ut_gen_expr) || ADD_TEST(suite_codegen, ut_gen_ass)) {
        CU_cleanup_registry();
        return CU_get_error();
    }

    //initialization
    if (ADD_TEST(suite_init, ut_config) || ADD_TEST(suite_init, ut_construct) || ADD_TEST(suite_init, ut_start_stop)) {
        CU_cleanup_registry();
        return CU_get_error();
    }
//LD expression parser
    if (ADD_TEST(suite_ld, ut_minmin) || ADD_TEST(suite_ld, ut_parse_ld_line)
